class Solution:
    ...
    def numberToWords(self, num: int) -> str:
        ...
        if num == 0:
            ...
            thousand = stringy[:-3]
            return str(
                self.hundConverter(int(thousand)) +
                (" Thousand " if int(thousand) != 0 else "") +
                self.hundConverter(int(hund))).strip(" ").replace("  ", " ")
            ...
            mil = stringy[:-6]
            return str(
                self.hundConverter(int(mil)) +
                (" Million " if int(mil) != 0 else "") +
                self.hundConverter(int(thousand)) +
                (" Thousand " if int(thousand) != 0 else "") +
                self.hundConverter(int(hund))).strip(" ").replace("  ", " ")
            ...
            bil = stringy[0]
            return str(
                self.hundConverter(int(bil)) +
                (" Billion " if int(bil) != 0 else "") +
                self.hundConverter(int(mil)) +
                (" Million " if int(mil) != 0 else "") +
                self.hundConverter(int(thousand)) +
                (" Thousand " if int(thousand) != 0 else "") +
